...
| Термин | Определение |
|--------|-------------|
| **1С** | Учётная система, создающая товарные чеки, приходные ордера и заказы на сборку |
...
- Кладовщик через ТСД сканирует коды маркировки с проверкой через ТС ПИоТ, разрешённые коды сохраняются в Oracle.
...
graph LR
   ...
   ONEC -->|1. Запрос на создание WMS-документа| API_ORCH
   API_ORCH -->|2. Сохраняет| ORACLE
   ONEC -->|3. Данные для чека| CASH_API
   ORACLE -->|4. Коды маркировки| CASH_API
   CASH_API --> ORCH
   ORCH -->|5. Проверка| TSPIOT
   TSPIOT -->|6. Результат| ORCH
   ORCH -->|7. Пробитие| KKT
   KKT -->|8. Чек| OFD
   ORCH -->|9. Статус| ONEC
```
...
4. **Сборка на складе с проверкой через ТС ПИоТ (первая проверка):**
   ...
   - 4.3. Кассовый модуль направляет код в ТС ПИоТ для проверки.
   - 4.4. ТС ПИоТ возвращает результат:
      - `ALLOWED` — код сохраняется в Oracle, привязанный к заказу.
      - `FORBIDDEN` — ТСД показывает ошибку, код не сохраняется, кладовщик заменяет товар.
   - 4.5. После сканирования всех кодов статус сборки обновляется.

5. Когда сборка завершена, 1С передаёт в кассовый модуль запрос на чек №2 (полный расчёт).

6. **Полный расчёт с повторной проверкой на кассе (вторая проверка):**
   - 6.1. Кассовый модуль запрашивает коды маркировки из Oracle.
   - 6.2. Кассовый модуль **повторно** отправляет коды в ТС ПИоТ для проверки актуального статуса.
...
### 7.4. Сохранение кодов с результатом проверки

- При сканировании кода на ТСД выполняется проверка через ТС ПИоТ.
- В Oracle сохраняются **только коды с результатом `ALLOWED`**.
- Для каждого сохранённого кода фиксируется:
    - Время сканирования
    - Идентификатор ТСД
    - Результат проверки (`ALLOWED`)
    - Время проверки через ТС ПИоТ
- Коды с результатом `FORBIDDEN` в Oracle не сохраняются.
...
### 8.2. Проверка кодов на этапе сборки (первая проверка)

1. ТСД сканирует код маркировки.
2. Код отправляется в ТС ПИоТ через Кассовый модуль.
3. ТС ПИоТ проверяет код в «Честном ЗНАКе» (онлайн) или в локальной базе (офлайн).
4. ТС ПИоТ возвращает результат: `ALLOWED` или `FORBIDDEN`.
5. При `ALLOWED` — код сохраняется в Oracle.
6. При `FORBIDDEN` — код не сохраняется, ТСД показывает ошибку.

### 8.3. Проверка кодов на этапе пробития чека (вторая проверка)
...
|----------|---------------------------|
| ТС ПИоТ недоступен на складе | Переход в офлайн-режим (если настроен) или блокировка сканирования |
| ТС ПИоТ недоступен на кассе | Блокировка продажи, уведомление администратора |
...
```mermaid
flowchart TD
    subgraph Склад["🏢 Этап 1. Сборка на складе"]
        A[ТСД сканирует код маркировки] --> B[Кассовый модуль отправляет код в ТС ПИоТ]
        B --> C{ТС ПИоТ проверяет код}
        C -->|ALLOWED| D[Код сохраняется в Oracle<br>привязанный к заказу]
        C -->|FORBIDDEN| E[ТСД показывает ошибку<br>Код НЕ сохраняется<br>Кладовщик заменяет товар]
        D --> F[Статус сборки обновлён]
        E --> F
    end

subgraph Касса["🏪 Этап 2. Пробитие чека"]
G[Сборка завершена] --> H[Кассовый модуль запрашивает<br>коды из Oracle]
H --> I[Кассовый модуль отправляет<br>коды в ТС ПИоТ повторно]
I --> J{ТС ПИоТ проверяет<br>актуальный статус кодов}
J -->|Все ALLOWED| K[Кассовый модуль формирует<br>отраслевой реквизит тег 1260]
K --> L[Пробитие чека 2<br>с кодами маркировки]
L --> M[Чек отправлен в ОФД]
J -->|Хотя бы один FORBIDDEN| N[Продажа БЛОКИРУЕТСЯ<br>Возврат ошибки в 1С]
end
...
|--------------|-------------------------------------------------------------------------------|--------------------------------------------------------------------------------|
| **1. Склад** | ТСД сканирует код → проверка через ТС ПИоТ                                    | `ALLOWED` → сохранение в Oracle<br>`FORBIDDEN` → ошибка, замена товара         |
| **2. Касса** | Кассовый модуль запрашивает коды из Oracle → повторная проверка через ТС ПИоТ | Все `ALLOWED` → чек пробит<br>Хотя бы один `FORBIDDEN` → продажа заблокирована |
...
| Режим | Описание |
|-------|----------|
| **Онлайн** | Чеки передаются в ОФД в реальном времени |
| **Офлайн** | Чеки сохраняются в буфере, передаются при восстановлении связи |
| **Смешанный** | Приоритет — онлайн, при недоступности — офлайн с автоматической отправкой |
...
| Переотправка | Автоматическая переотправка чеков при сбоях |
...
**Шаг 3. Сборка на складе с проверкой через ТС ПИоТ**
- 1С отправляет задание на ТСД кладовщику (через WMS).
- Кладовщик сканирует коды маркировки.
- **Каждый код сразу проверяется через ТС ПИоТ.**
- В Oracle сохраняются **только разрешённые коды**.
- Если код запрещён — ТСД показывает ошибку, кладовщик заменяет товар.
- Статус сборки обновляется.

**Шаг 4. Полный расчёт с повторной проверкой**
- Когда сборка завершена, 1С передаёт в кассовый модуль запрос на чек №2.
- Кассовый модуль запрашивает коды маркировки из Oracle.
- **Кассовый модуль повторно проверяет коды через ТС ПИоТ.**
...
- Кладовщик сканирует коды маркировки.
- Коды сохраняются в Oracle, привязанные к заказу.
- Статус сборки обновляется.
...
| Требование | Описание |
|------------|----------|
| **Офлайн-буфер** | При потере связи с ОФД чеки сохраняются и отправляются автоматически при восстановлении |
| **Ретраи** | При ошибке отправки чека — автоматические повторные попытки (до 5 раз) |
| **Лимит времени** | Максимальное время ожидания ответа от ОФД — 30 секунд |
| **Защита от дублей** | Защита от повторной отправки одного чека (идемпотентность) |
| **Ручное вмешательство** | При невозможности автоматической отправки — уведомление администратора |
...
- [ ] При сканировании кода на ТСД выполняется проверка через ТС ПИоТ, в Oracle сохраняются только разрешённые коды.
