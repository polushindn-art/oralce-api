...
| **Приоритет**                        | Очерёдность выполнения                   | [см. таблицу приоритетов](#приоритет-документа)            |
...
| **Способ распределения** | Определяет, как документ делится между исполнителями | [см. таблицу способов распределения] |

---

#### Способ распределения (distribution_type)

**Что это:** Единый для всего документа способ распределения товаров между исполнителями.

**Зачем нужен:**
- Определяет, по какому принципу делить документ между кладовщиками
- Обеспечивает единообразие распределения в рамках одного документа
- Позволяет гибко настраивать логику работы склада

| Способ | Код | Описание | Когда использовать | Пример |
|--------|-----|----------|-------------------|--------|
| **По зонам склада** | `ZONE` | Разделение по физическим зонам склада | Когда склад разделен на зоны (стеллажи, ряды) | Иванов → зона А, Петров → зона Б |
| **По весу** | `WEIGHT` | Разделение по весу товаров | Когда есть тяжелые и легкие товары, разные сотрудники имеют разные возможности | Сильные → тяжелые, слабые → легкие |
| **По объёму** | `VOLUME` | Разделение по объёму товаров | Когда есть крупногабаритные товары | Крупные → на отдельного сотрудника |
| **По товарным группам** | `PRODUCT_GROUP` | Разделение по категориям товаров | Когда сотрудники специализируются на разных товарах | Иванов → электроника, Петров → мебель |
| **По секциям** | `SECTION` | Разделение по секциям магазина | Для магазинов с разными секциями | Иванов → электроника, Петров → одежда |


##### Влияние на работу

| Что меняется | Описание |
|--------------|----------|
| **Назначение исполнителей** | Исполнители должны иметь параметры, соответствующие способу распределения |
| **Фильтрация на ТСД** | Каждый исполнитель видит только свои позиции согласно способу распределения |
| **Контроль выполнения** | Статус документа завершается только когда все исполнители завершили свою часть |
---
...
| **Журнал с кумулятивным итогом** | Все фактические операции только INSERT, состояние вычисляется как сумма | Полный аудит, отказоустойчивость, простота синхронизации |

### 3.1.1. Архитектурный принцип "Журнал с кумулятивным итогом" (Event Sourcing)

Система должна работать по принципу **"неизменяемого журнала операций"**. Все фактические действия на складе (сканирования, перемещения, списания) фиксируются как **отдельные, неизменяемые записи** в таблице фактических операций. Текущее состояние документа вычисляется как кумулятивная сумма всех операций.

#### Основные правила

| Правило | Описание | Обоснование |
|---------|----------|-------------|
| **Только добавление (Append-Only)** | Фактические операции никогда не изменяются и не удаляются | Обеспечивает полный аудит, позволяет восстановить состояние на любой момент |
| **Кумулятивный итог (running_quantity)** | Каждая запись хранит текущее состояние после операции | Даёт мгновенный доступ к состоянию без суммирования всех операций |
| **Неизменяемость (Immutable)** | После записи факт не может быть изменён | Корректировки выполняются только через новую операцию |
| **Идемпотентность** | Повторная отправка одной операции не создаёт дубликат | Безопасная синхронизация при разрывах связи |

#### Преимущества

| Преимущество | Описание |
|--------------|----------|
| ✅ **Полная история** | Каждая операция сохраняется, можно восстановить цепочку действий |
| ✅ **Отказоустойчивость** | Данные не теряются, даже если синхронизация прервалась |
| ✅ **Простая синхронизация** | Только INSERT, никаких конфликтов при слиянии данных |
| ✅ **Высокая производительность** | Кумулятивный итог даёт мгновенный доступ к текущему состоянию |
| ✅ **Безопасность** | Невозможно случайно изменить или удалить уже совершённую операцию |
---
...
| `WMSFACT`      | Фактические операции (журнал движений)           |
...
### 11.7. Таблица фактических операций (WMSFACT)

**Назначение:** Хранение всех фактических операций по документу. Реализует принцип "неизменяемого журнала операций".

| Поле | Тип | Назначение                                            |
|------|-----|-------------------------------------------------------|
| `rn` | NUMBER(19) PK | Идентификатор записи                                  |
| `wmspec_rn` | NUMBER(19) FK | Ссылка на строку спецификации (WMSSPEC)               |
| `quantity` | NUMBER(10) | Изменение количества (+ приёмка, - отгрузка/списание) |
| `running_quantity` | NUMBER(10) | Кумулятивный итог после операции                      |
| `result_status` | VARCHAR2(20) | GOOD / UNDEFINED                                      |
| `defect_reason` | VARCHAR2(500) | Причина разбора (если UNDEFINED)                         |
| `code` | VARCHAR2(50) | Отсканированный код (MARK / EAN / NULL)               |
| `code_type` | VARCHAR2(20) | MARK / EAN / NULL                                     |
| `picker_id` | VARCHAR2(50) | ID исполнителя                                        |
| `device_id` | VARCHAR2(50) | ID ТСД                                                |
| `executed_at` | TIMESTAMP | Время выполнения                                      |
| `local_id` | VARCHAR2(50) | Уникальный ID из Room (для дедупликации)              

**Правила:**
- Только INSERT (никогда UPDATE)
- `running_quantity` = предыдущее значение + `quantity`
- Текущее состояние = `running_quantity` из последней записи
