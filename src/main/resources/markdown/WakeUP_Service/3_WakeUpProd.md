...
- Создать Intent с component name = com.wms.app/.MainActivity
- Добавить флаг FLAG_ACTIVITY_NEW_TASK
- Вызвать startActivity

**Способ 2 — если WMS уже работает (в фоне или на переднем плане):**

- Создать Intent с action = "com.wms.WAKEUP"
- Вызвать sendBroadcast

**Способ 3 — комбинированный:**

- Сначала отправить Broadcast (для уже работающего приложения)
- Если через 1 секунду нет ответа — запустить Activity

**Примечание:** WakeUp НЕ передаёт никаких данных (task_id, priority, title, message). WMS приложение после пробуждения самостоятельно запрашивает у REST API список своих заданий, сортирует по приоритету и открывает самое приоритетное.
